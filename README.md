...
- y - координата Y (мм);
- z - координата Z (мм);
- alpha - угол aX (градусы);
- beta - угол Ay (градусы);
- gamma - угол Az (градусы). <br />

Результатом работы функции будет список из 6-ти углов редукторов (радианы). Если заданные на входе параметры приведут к выходу за диапазон работы платформы, Вам вернут сообщение об этом (будет конкретно написано, какая часть платформы сломана). 
