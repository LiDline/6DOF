...
# Локальные СК верхней плиты
local_coordinate_system_upper_point = zeros((6, 4, 4, 1))
# Локальные СК точек пересечения
local_coordinate_system_crosspoints_after_rotate = zeros((6, 4, 4, 1))
...
def leg_ball_corners(matrix_move_lower_plate, upper_points, matrix_move_upper_plate):
    for i in range(6):
            ...
            if border_of_arm_angle[0] <= cross_matrix[3]*180/pi <= border_of_arm_angle[1]:
                ...
                print(
                    f'Угол рычага {i+1} = {cross_matrix[3]*180/pi} => не входит в допустимый диапазон')
    ...
    # _____________________________________________________________________________________________________________________________________________________________
            ...
            coordinates_crosspoints_from_upper_points[i] = linalg.inv(
                matrix_move_upper_plate[i]).dot(global_cross_coordinates[i])
            ...
            angles_between_crosspoints_and_upper_points[i] = atan2(
                coordinates_upper_points_from_crosspoints[i][1][0], coordinates_upper_points_from_crosspoints[i][0][0])
            # Для верхней точки плиты тяга няходится в 3-ей четверти, поэтому ближе повернуть плоскость YZ, а не XZ
            angles_between_upper_points_and_crosspoints[i] = atan2(
                coordinates_crosspoints_from_upper_points[i][0][0], coordinates_crosspoints_from_upper_points[i][1][0])

            # Матрицы поворота СК (чтобы тяга лежала в плоскости ZX)
            matrix_rotate_cs_crosspoints_to_upper_points[i] = rotation_matrix(
                0, 0, angles_between_crosspoints_and_upper_points[i])
            # Матрицы поворота СК (чтобы тяга лежала в плоскости ZY)
            matrix_rotate_cs_upper_points_to_crosspoints[i] = rotation_matrix(
                0, 0, -angles_between_upper_points_and_crosspoints[i])

            # Создадим общую матрицу перемещения до точки пересечения с последующим поворотом до XZ
            matrix_move_to_new_cs_of_crosspoints[i] = matrix_move_crosspoints[i].dot(
                matrix_rotate_cs_crosspoints_to_upper_points[i])
            # Создадим общую матрицу перемещения до верхней точки с последующим поворотом до YZ
            matrix_move_to_new_cs_of_upper_points[i] = matrix_move_upper_plate[i].dot(
                matrix_rotate_cs_upper_points_to_crosspoints[i])

            # Узнаем координаты верхнего шарнира точки пересечения в новой СК
            coordinates_upper_points_from_crosspoints_in_new_planes[i] = linalg.inv(
                matrix_move_to_new_cs_of_crosspoints[i]).dot(upper_points[i])
            # Узнаем координаты нижнего шарнира относительно верхнего в новой СК
            coordinates_crosspoints_from_upper_points_in_new_planes[i] = linalg.inv(
                matrix_move_to_new_cs_of_upper_points[i]).dot(global_cross_coordinates[i])

            # Найдём углы
            angle_lower_joint[i] = atan2(coordinates_upper_points_from_crosspoints_in_new_planes[i]
                                         [2][0], coordinates_upper_points_from_crosspoints_in_new_planes[i][0][0])
            angle_upper_joint[i] = atan2(coordinates_crosspoints_from_upper_points_in_new_planes[i]
                                         [2][0], coordinates_crosspoints_from_upper_points_in_new_planes[i][1][0])

            for j in range(len(global_coordinate_system)): 
                local_coordinate_system_upper_point[i][j] = matrix_move_to_new_cs_of_upper_points[i].dot(matrix_1_4(global_coordinate_system[j][0], 
                                                                global_coordinate_system[j][1], global_coordinate_system[j][2])) 
                local_coordinate_system_crosspoints_after_rotate[i][j] = matrix_move_to_new_cs_of_crosspoints[i].dot(matrix_1_4(global_coordinate_system[j][0],   # Локальные СК точек пересечения
                                                            global_coordinate_system[j][1], global_coordinate_system[j][2]))    
                
    return arm_angle, angle_lower_joint, angle_upper_joint, global_cross_coordinates, local_cross_coordinates, local_coordinate_system_upper_point, local_coordinate_system_crosspoints_after_rotate