from numpy import array, sqrt, float64, nan
...
def cross_normal_to_oz(matrix, R, r):   # https://hedgedoc.auto-sys.su/JAqGGd3JRn-qmUJVMdjODg
    ...
    if D >= 0:
        ...
        if t_2 >= t_1:
            return  array((x_1, y_1, 0, t_1), dtype = float64)   # X = 0, т.к. считаем, что окружность лежит в плоскости ZX 
        elif t_1 > t_2:
            return  array((x_2, y_2, 0, t_2), dtype = float64)
    else:
        return array((nan, nan, nan, nan), dtype = float64)